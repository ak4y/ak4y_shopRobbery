...
AddEventHandler("mythic_progbar:client:progress", function(action, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or mythic_action.useWhileDead then
        if not isDoingAction then
            ...
            TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = 'You are currently performing another transaction..' })
        end
    else
        TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = 'Cannot Perform An Action While Dead' })
    end
end)
...
function DisableActions(ped)
    ...
    if mythic_action.controlDisables.disableMovement then
        ...
        DisableControlAction(0, 44, true) -- disable sprint
